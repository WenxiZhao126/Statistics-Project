...
df <- cbind(df, output[,-1])  #- added -1 here to drop one level
...
# squared sqft_lot, squared bathrooms and interactions of sqft_lot and bathrooms
df.poly <- df
...
poly.fit <- lm(log(price) ~., data=df.poly)
summary(poly.fit)
# sqft_lot2 is not significant. we will keep only quadratic terms of bathrooms.
